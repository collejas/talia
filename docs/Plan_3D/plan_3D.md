...
- El formulario incluye campos esenciales (nombre, tipo, matriz de país/estado/municipio/código postal/colonia, precio, status, `height`, `min_height`, `levels`, `color`, referencia a línea/familia/modelo) y controles para guardar, limpiar, centrar y validar la geometría. `metadata` queda como extensión opcional, no como campo operativo.
