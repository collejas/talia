-- Inbox: add source/channel/batch/campaign filters to threads RPC.

CREATE INDEX IF NOT EXISTS idx_mensajes_datos_source
    ON public.mensajes ((lower((datos ->> 'source'))))
    WHERE datos ? 'source';

CREATE INDEX IF NOT EXISTS idx_mensajes_datos_batch_id
    ON public.mensajes ((datos ->> 'batch_id'))
    WHERE datos ? 'batch_id';

CREATE INDEX IF NOT EXISTS idx_mensajes_datos_campana_id
    ON public.mensajes ((datos ->> 'campana_id'))
    WHERE datos ? 'campana_id';
