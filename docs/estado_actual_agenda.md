...
- `schedule_demo` ahora llama `storage.schedule_demo_cita` (RPC `fn_cita_schedule_json_payload_v2`) tras asegurar la tarjeta. `reschedule_demo` delega en `storage.reschedule_demo_cita` (`fn_cita_reschedule`).
...
- `fn_cita_schedule_json_v1` (consumida vía `fn_cita_schedule_json_payload_v2`) y `fn_cita_reschedule` delegan en `fn_cita_upsert`, pero la constraint `citas_calendario_range_excl` evita empalmes para calendarios con capacidad 1.
