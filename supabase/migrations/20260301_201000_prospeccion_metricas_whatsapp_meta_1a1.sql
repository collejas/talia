...
envios_base AS (
    SELECT
        ...
        NULLIF(
            lower(
                COALESCE(
                    NULLIF(btrim(e.detalle->>'error_code'), ''),
                    NULLIF(btrim(e.error), '')
                )
            ),
            ''
    ...
    SELECT
        ...
        COUNT(*) FILTER (
            WHERE (
                eb.canal = 'whatsapp'
                AND (
                    eb.estado IN ('entregado', 'leido', 'completado', 'respondido')
                    OR (eb.estado = 'enviado' AND eb.provider_error_code IS NULL)
                )
            )
            OR (
                eb.canal <> 'whatsapp'
        ...
        )::bigint AS envios_totales,
        COUNT(*) FILTER (
            WHERE (
                eb.canal = 'whatsapp'
                AND (
                    eb.estado IN ('enviado', 'entregado', 'leido', 'completado', 'respondido')
                    AND (eb.estado <> 'enviado' OR eb.provider_error_code IS NULL)
                )
            )
            OR (
                (eb.canal <> 'whatsapp' OR eb.canal IS NULL)
                AND eb.estado IN ('enviado', 'entregado', 'leido', 'completado', 'respondido')
            )
        ...
        COUNT(*) FILTER (
            WHERE eb.estado IN ('fallido', 'error', 'failed', 'undelivered')
               OR (eb.canal = 'whatsapp' AND eb.estado = 'enviado' AND eb.provider_error_code IS NOT NULL)
        )::bigint AS envios_fallidos,
...
sesion_atribucion AS (
    ...
    GROUP BY eb.campana_id, eb.template_uuid, COALESCE(lower(t.slug), eb.template_slug)
)
SELECT
    a.campana_id,
    a.campana_nombre,
