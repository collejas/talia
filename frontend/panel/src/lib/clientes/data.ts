  ...
  return rows.map((row, index) => {
    ...
    return {
      ...
      raw: {
        ...
        telefono: row.contacto?.telefono_e164,
        correo: row.contacto?.correo,
        ultima_compra_en: row.ultima_compra_en,
        ultimo_pago_en: row.ultimo_pago_en,
        proxima_actividad_en: row.proxima_actividad_en,
