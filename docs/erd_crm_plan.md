...
- `backend/app/api/routes/panel.py` continúa leyendo y escribiendo `lead_tarjetas`, `lead_etapas` y `contactos` directamente desde Supabase (ej. `_fetch_lead_for_quote`), por lo que las cotizaciones y movimientos dependen del modelo viejo. 【backend/app/api/routes/panel.py:1589-1624】
- `backend/app/services/storage.py` mantiene la función `ensure_lead_tarjeta`, que crea/actualiza tarjetas via `/rest/v1/lead_tarjetas` para cada conversación entrante; aún no existe un equivalente en `oportunidades` o `actividades`. 【backend/app/services/storage.py:1792-1858】
- El nuevo router `/crm` y `CRMRepository` sí exponen todo el ERD multi-tenant (cuentas, oportunidades, actividades, tickets, productos, campañas, leads, notas, audit logs), pero todavía no respaldan los flujos principales; hoy sólo las vistas de reporting los consumen. 【backend/app/api/routes/crm.py:1-1178】【backend/app/repositories/crm.py:1-746】
- En el frontend ya existe `callCrmApi` y páginas para `/crm` (cuentas, oportunidades, actividades, tickets, campañas, leads, archivos, notas), pero todas son lecturas renderizadas con `ClientDataTable`. La creación/edición de leads, embudo, agenda y settings siguen utilizando `callSupabaseRest` / `callSupabaseRpc`, escribiendo directamente en `lead_tarjetas` y `contactos`. 【frontend/panel/src/lib/api/crm.ts:1-192】【frontend/panel/src/app/crm/oportunidades/page.tsx:1-23】【frontend/panel/src/lib/crm/opportunities.ts:39-64】【frontend/panel/src/lib/leads/supabase.ts:67-194】【frontend/panel/src/lib/embudo/actions.ts:378-539】
- Persisten módulos de configuración que dependen del acceso directo a Supabase (por ejemplo `frontend/panel/src/app/settings/email/actions.ts:145-199` y `frontend/panel/src/app/settings/reminders/actions.ts:51-106`), y las migraciones en `supabase/migrations_tmp/20251026_210000_leads_kanban.sql:128-205` siguen describiendo únicamente el tablero legacy sin `organizacion_id`.

## Estado actual resumido
- **Base de datos:** el dump `backups/postgres_20251122_221914/postgres_20251122_221914_schema.sql` ya contiene el ERD multi-tenant completo con RLS, pero los artefactos `lead_*` siguen siendo la fuente operativa. Falta migrar datos y habilitar doble escritura antes de retirar esas tablas.
- **Backend:** FastAPI expone `/crm` con un cliente Supabase de servicio y ya incluye APIs para pipeline, archivos, notas, plantillas (email/cotizaciones) y recordatorios; sin embargo, otros módulos (`panel.py`, inbox) continúan leyendo/escribiendo `lead_tarjetas`, por lo que todavía existe lógica acoplada al modelo legacy. 【backend/app/api/routes/panel.py:1589-1624】【backend/app/services/storage.py:1792-1858】
- **Frontend:** el embudo, las vistas de settings (email, formato de cotización y recordatorios) y los listados `/crm/*` ya consumen `callCrmApi`, pero otros flujos (contactos, inbox, prospección, settings/catalogo) siguen llamando a Supabase directamente. 【frontend/panel/src/lib/api/crm.ts:1-192】【frontend/panel/src/lib/leads/supabase.ts:67-194】
- **Infra/RLS:** Las políticas por `organizacion_id` están disponibles y funcionales en las tablas nuevas, pero no protegen todos los flujos actuales porque varias vistas del panel continúan conectándose directo a Supabase con la anon key del tenant.
...
1. **Cliente Supabase de servicio:** encapsular en `backend/app/repositories/crm_repository.py` todas las llamadas a Supabase usando el service role y el nuevo `organizacion_id`. _Listo; falta usarlo fuera de `backend/app/api/routes/crm.py` para reemplazar los accesos directos en `panel.py` y `storage.py`._
...
- **Settings:** las plantillas de email (`/settings/email`), el formato de cotizaciones (`/settings/formato-cotizacion`) y los recordatorios (`/settings/reminders`) ya se leen/escriben vía `/crm/settings/*`, eliminando el uso de `callSupabaseRest` en estas secciones.
- **Pendientes inmediatos:** (1) propagar el nuevo flujo a los demás módulos que siguen en Supabase directo (settings/catalogo, contactos, inbox, prospección) para cerrar completamente el modelo legacy; (2) agregar manejo de conflictos de etapa (`409 opportunity_stage_conflict`) en la UI para soportar edición concurrente; (3) enlazar los contadores de visitantes (`visitantes_sin_chat`) con los futuros dashboards.
