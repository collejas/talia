...
export default async function OnboardingStepPage({ params }: { params: Promise<{ step: string }> }) {
  ...
  const content = (() => {
    switch (step.id) {
      ...
      case "busqueda":
        return <TenantBusquedaSettings tenantId={tenantId} initialValues={{ denue_base_url: text(denue, "base_url") }} hasToken={hasSecret("denue.token")} hasGoogleApiKey={hasSecret("google.places_api_key")} showRequiredMarkers />
      case "whatsapp":
