...
4. **Seed de etapas con códigos canónicos:** _cubierto con `supabase/migrations/20260201_123000_seed_pipeline_stages.sql`, la vista `organizaciones_missing_etapas_pipeline` y la función `check_missing_pipeline_stages()`. Configura un cron (Supabase Scheduled Function, GitHub Action, etc.) que ejecute `select public.check_missing_pipeline_stages();` y envíe una alerta si la consulta falla con `missing_pipeline_stages`._
