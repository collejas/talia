...
- `backend/app/services/storage.py` ya enruta los registros de webchat y WhatsApp mediante `CRMRepository`, pero `fetch_conversation`, `get_webchat_contact_id`, `fetch_webchat_session_id` y otras lecturas siguen usando el service role directo; falta llevar esos caminos a `/crm` para completar el aislamiento multi-tenant. 【backend/app/services/storage.py:133-220】
...
4. **Sincronización de conversaciones:** migrar las lecturas y bitácoras pendientes (consultas a `conversaciones`, identidades de canal y cargas de adjuntos) para que `storage.py` invoque helpers equivalentes en `/crm`, completando el corte con las tablas legacy.
