...
- **Backend:** FastAPI ahora expone `/crm` con un cliente Supabase de servicio, pero el resto de la API (panel, inbox, sincronización) continúa leyendo/escribiendo `lead_tarjetas`, de modo que la lógica de negocio aún no usa `cuentas`/`oportunidades`. 【backend/app/api/routes/panel.py:1589-1624】【backend/app/services/storage.py:1792-1858】
- **Frontend:** Next.js ya tiene un cliente `callCrmApi` y vistas `/crm/*` para listar entidades, aunque la mayoría de las interacciones (embudo, leads, configuraciones) siguen llamando a Supabase directamente desde el navegador. 【frontend/panel/src/lib/api/crm.ts:1-192】【frontend/panel/src/lib/leads/supabase.ts:67-194】
- **Infra/RLS:** Las políticas por `organizacion_id` están disponibles y funcionales en las tablas nuevas, pero no protegen los flujos actuales porque la UI continúa conectándose directamente a Supabase con la anon key del tenant actual.
...
- **Pendientes inmediatos:** (1) migrar `searchEmbudoContacts` y las actualizaciones de contacto a los endpoints `/crm/contacts`, eliminando `callSupabaseRest`; (2) propagar el nuevo flujo a los drawers/acciones de otras vistas (lead drawer, recordatorios, demo scheduler) para que ningún flujo escriba en tablas legacy; (3) agregar manejo de errores/conflictos de etapa (`409 opportunity_stage_conflict`) en la UI para soportar edición concurrente; (4) enlazar los contadores de visitantes (`visitantes_sin_chat`) con los futuros dashboards.
