drop function if exists public.prospeccion_campana_whatsapp_metricas_rango(uuid, timestamptz, timestamptz, integer, integer);

create function public.prospeccion_campana_whatsapp_metricas_rango(
...
delivery_rollup as (
    select
        ...
        count(*) filter (where evento in ('entregado', 'delivered'))::bigint as mensajes_entregados,
        count(*) filter (where evento in ('leido', 'read'))::bigint as mensajes_leidos,
        count(*) filter (where evento in ('fallido', 'failed', 'undelivered'))::bigint as mensajes_fallidos
...
comment on function public.prospeccion_campana_whatsapp_metricas_rango(uuid, timestamptz, timestamptz, integer, integer)
    is 'Resume metricas de campañas WhatsApp por organizacion y agrega desglose de entregado/leido/fallido/sin traza via eventos_entrega.';
