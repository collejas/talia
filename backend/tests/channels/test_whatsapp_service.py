...
async def test_structured_fast_path_is_bypassed_when_agenda_disabled(monkeypatch) -> None:
    reply = await service._maybe_build_structured_fast_reply(
        ...
        persona_record={"id": "persona-disabled", "nombre_completo": "Antonia Urdueña"},
        conversation_meta={"last_response_id": "resp-disabled", "inbox_context": {}},
        message=schemas.WhatsAppIncomingMessage(
            message_sid="SM-company-disabled",
            from_number="whatsapp:+521111111111",
            to_number="whatsapp:+521000000000",
            body="IMLUX",
            wa_id="521111111111",
...
@pytest.mark.asyncio
async def test_generate_assistant_reply_recovers_incomplete_response(monkeypatch) -> None:
    """Una respuesta incompleta debe recuperarse sin caer al fallback."""
    ...
    assert reply.text == "Respuesta recuperada"
    assert reply.response_id == "resp-recovered"
