...
- `backend/app/services/storage.py` ya enruta los registros y lecturas básicas de conversaciones mediante `CRMRepository`, pero utilidades de otros módulos (subida de PDFs de cotización, logos, documentos de clientes, consultas de analytics y descargas masivas) siguen consultando Supabase directo con el service role; falta mover esos caminos a `/crm` para completar el aislamiento multi-tenant. 【backend/app/services/storage.py:520-900】
...
4. **Sincronización de conversaciones:** migrar los uploads/reportes restantes (PDFs de cotización, logos, documentos de clientes y analytics) para que `storage.py` invoque helpers equivalentes en `/crm`, completando el corte con las tablas legacy.
