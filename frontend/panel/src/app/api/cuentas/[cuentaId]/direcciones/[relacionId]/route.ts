...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const response = await callCrmApi<UnknownRecord>(
    `/crm/cuentas/${encodeURIComponent(cuentaId)}/direcciones/${encodeURIComponent(relacionId)}`,
    {
      method: "PATCH",
      body: payload,
      withUserToken: true,
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_direccion_update_failed" },
...
export async function DELETE(_: NextRequest, context: RouteContext) {
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_direccion_delete_failed" },
      { status: response.status ?? 502 },
    );
  }
