...
-- Evitar duplicados nuevos por busqueda:
-- 1) normaliza la identidad con dedupe_key,
-- 2) reutiliza filas existentes por organizacion/fuente/external_id o dedupe_key,
-- 3) registra cada aparicion en la tabla puente.

CREATE UNIQUE INDEX IF NOT EXISTS resultados_org_fuente_dedupe_uidx
    ON public.resultados (organizacion_id, fuente, dedupe_key)
...
begin
    ...
    appearances AS (
        ...
        ON CONFLICT (organizacion_id, busqueda_id, resultado_id) DO UPDATE
