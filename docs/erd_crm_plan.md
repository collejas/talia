...
- Las lecturas de conversación y las identidades webchat también pasan por `CRMRepository`, así que `fetch_conversation`, `get_webchat_contact_id` y `fetch_webchat_session_id` ya no construyen llamadas directas a Supabase. 【backend/app/repositories/crm.py:1227-1285】【backend/app/services/storage.py:123-162】
...
- `backend/app/services/storage.py` ya enruta los registros y lecturas básicas de conversaciones mediante `CRMRepository`, pero funciones como `resolve_webchat_conversation_from_session` y otros fetchers aún consultan `conversaciones`/estadísticas directamente con el service role; falta llevar esos caminos a `/crm` para completar el aislamiento multi-tenant. 【backend/app/services/storage.py:165-360】
