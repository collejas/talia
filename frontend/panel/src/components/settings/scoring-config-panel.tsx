  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3 rounded-lg border p-4">
        <h2 className="text-lg font-semibold">Paso 3. Repreguntas (respaldo)</h2>
        <p className="text-sm text-muted-foreground">
          La edición principal ya vive dentro de cada pregunta en Paso 2. Este bloque queda como vista y edición auxiliar.
        </p>
      ...
      <section className="space-y-3 rounded-lg border p-4">
        <h2 className="text-lg font-semibold">Paso 4. Reglas de puntuación (respaldo)</h2>
        <p className="text-sm text-muted-foreground">
          Las reglas se pueden editar dentro de cada pregunta en Paso 2. Aquí queda la vista general.
        </p>
        ...
        <div className="grid gap-2 md:grid-cols-7">
          <select
            ...
            onChange={(event) => syncRuleQuestionSelection(event.target.value)}
          >
            <option value="">Selecciona una pregunta</option>
            {questionOptions.map((option) => (
              <option key={option.id} value={option.id}>
          ...
          <select
            ...
            onChange={(event) => {
              ...
              const fieldKey = findQuestionFieldKey(ruleQuestionId);
              const options = fieldKey ? resolveRuleValueOptions(fieldKey) : [];
              if (options.length) {
                setRuleMatchValueMode("preset");
                setRuleMatchValue(options.includes(ruleMatchValue) ? ruleMatchValue : options[0]);
                return;
              }
              setRuleMatchValueMode("custom");
            }}
          ...
          {(() => {
            const fieldKey = findQuestionFieldKey(ruleQuestionId);
            const options = fieldKey ? resolveRuleValueOptions(fieldKey) : [];
            const canUsePresetValues = options.length > 0 && (ruleType === "equals" || ruleType === "in_set");
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    const itemRuleValueOptions = resolveRuleValueOptions(findQuestionFieldKey(item.question_id) ?? "");
                    if (
                      itemRuleValueOptions.length > 0 &&
                      itemRuleValueOptions.includes(item.match_value ?? "") &&
                      (item.rule_type === "equals" || item.rule_type === "in_set")
                    ) {
                      setRuleMatchValueMode("preset");
                    } else {
                      setRuleMatchValueMode("custom");
