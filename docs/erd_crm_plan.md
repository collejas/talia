...
- El router `/crm` y `CRMRepository` ya abastecen agenda, visitas, prospección, settings, contactos e inbox, pero los flujos de ventas/cotizaciones siguen dependiendo de `panel.py` y de RPC legacy (`lead_tarjetas`, `panel_lead_*`), por lo que aún no se elimina la doble escritura en pipeline. 【backend/app/api/routes/crm.py:1-3704】【backend/app/api/routes/panel.py:1589-1900】【backend/app/services/storage.py:1792-1858】
...
- **Backend:** FastAPI expone `/crm` con un cliente Supabase de servicio y ya incluye APIs para pipeline, archivos, notas, plantillas (email/cotizaciones), inbox y recordatorios; sin embargo, `panel.py` todavía maneja las cotizaciones y automatismos que escriben directamente en `lead_tarjetas`, por lo que persiste lógica acoplada al modelo legacy. 【backend/app/api/routes/panel.py:1589-1900】【backend/app/services/storage.py:1792-1858】
