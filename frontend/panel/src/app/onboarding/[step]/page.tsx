...
type Progress = {
  ...
  google_places_decision: GooglePlacesDecision
  correo?: {
...
export default async function OnboardingStepPage({ params }: { params: Promise<{ step: string }> }) {
  ...
  const content = (() => {
    switch (step.id) {
      case "organizacion":
        return <TenantOrganizationInfoForm tenantId={tenantId} info={data} showActiveToggle={false} showOnboardingState={false} showRequiredMarkers />
      ...
      case "agenda":
        return <TenantCalendarSettings tenantId={tenantId} initialValues={{ agenda_enabled: bool(agenda, "enabled") ?? true, calendar_timezone: text(webchatCalendar, "timezone") ?? "", calendar_default_days: number(webchatCalendar, "default_days"), calendar_hold_minutes: number(webchatCalendar, "hold_minutes"), calendar_provider: text(calendar, "provider") ?? "", calendar_server_url: text(calendar, "server_url") ?? "", calendar_server_url_alternate: text(calendar, "server_url_alternate") ?? "", calendar_server_port: number(calendar, "server_port"), calendar_full_calendar_url: text(calendar, "full_calendar_url") ?? "", calendar_full_contact_list_url: text(calendar, "full_contact_list_url") ?? "", zoom_enabled: bool(record(config.zoom), "enabled") ?? false, zoom_host_email: text(record(config.zoom), "host_email") ?? "", zoom_default_duration_minutes: number(record(config.zoom), "default_duration_minutes"), zoom_auto_create_meeting: bool(record(config.zoom), "auto_create_meeting") ?? true }} showZoomChoice zoomDecision={progressResp.data.zoom_decision} showRequiredMarkers />
      case "correo":
        return <><EmailSetupChecklist status={progressResp.data.correo} /><TenantEmailServicePanel data={emailResp.ok ? emailResp.data : null} configurationComplete={progressResp.data.correo?.completado === true} createDomainAction={tenantActionsLib.createTenantEmailDomainAction} verifyDomainAction={tenantActionsLib.verifyTenantEmailDomainAction} removeDomainAction={tenantActionsLib.removeTenantEmailDomainAction} updateSenderAction={tenantActionsLib.updateTenantEmailSenderAction} showRequiredMarkers /><TenantMailSettings tenantId={tenantId} initialValues={{ mail_incoming_server: text(mail, "incoming_server"), mail_incoming_port_imap: number(mail, "incoming_port_imap"), mail_outgoing_server: text(mail, "outgoing_server"), mail_outgoing_port_smtp: number(mail, "outgoing_port_smtp"), mail_use_ssl: bool(mail, "use_ssl"), mail_use_tls: bool(mail, "use_tls") }} showRequiredMarkers /></>
      case "busqueda":
        return <TenantBusquedaSettings tenantId={tenantId} initialValues={{ denue_base_url: text(denue, "base_url") }} hasToken={hasSecret("denue.token")} hasGoogleApiKey={hasSecret("google.places_api_key")} googlePlacesDecision={progressResp.data.google_places_decision} showRequiredMarkers />
      case "whatsapp":
        return <><MetaAssistedConnectionPanel initialConnection={metaResp.ok ? metaResp.data : null} businessId={process.env.META_TALIA_BUSINESS_ID ?? "1358726956043196"} /><TenantWhatsAppMetaSettings tenantId={tenantId} initialValues={{ whatsapp_provider: "meta", whatsapp_send_seller_data_to_customer: bool(whatsapp, "send_seller_data_to_customer") ?? false, whatsapp_prompt_id: text(whatsapp, "prompt_id"), whatsapp_prompt_version: text(whatsapp, "prompt_version"), whatsapp_welcome_document_prompt_version: text(whatsapp, "welcome_document_prompt_version"), whatsapp_location_href: text(whatsapp, "location_href"), whatsapp_assistant_id: text(whatsapp, "assistant_id"), whatsapp_inactivity_minutes: number(whatsapp, "inactivity_minutes"), whatsapp_reengage_minutes: number(whatsapp, "reengage_minutes"), whatsapp_reengage_max_attempts: number(whatsapp, "reengage_max_attempts"), whatsapp_escalate_minutes: number(whatsapp, "escalate_minutes"), whatsapp_meta_phone_number_id: text(whatsappMeta, "phone_number_id") }} /><TenantWhatsAppRoutes tenantId={tenantId} routes={routes} /><TenantWhatsAppSettings tenantId={tenantId} initialValues={{ whatsapp_provider: "meta", whatsapp_send_seller_data_to_customer: bool(whatsapp, "send_seller_data_to_customer") ?? false, whatsapp_prompt_id: text(whatsapp, "prompt_id"), whatsapp_prompt_version: text(whatsapp, "prompt_version"), whatsapp_welcome_document_prompt_version: text(whatsapp, "welcome_document_prompt_version"), whatsapp_location_href: text(whatsapp, "location_href"), whatsapp_assistant_id: text(whatsapp, "assistant_id"), whatsapp_inactivity_minutes: number(whatsapp, "inactivity_minutes"), whatsapp_reengage_minutes: number(whatsapp, "reengage_minutes"), whatsapp_reengage_max_attempts: number(whatsapp, "whatsapp_reengage_max_attempts"), whatsapp_escalate_minutes: number(whatsapp, "escalate_minutes"), whatsapp_meta_phone_number_id: text(whatsappMeta, "phone_number_id") }} /><TenantWhatsAppProspeccionSettings tenantId={tenantId} initialValues={{ whatsapp_prospeccion_prompt_id: text(whatsapp, "prospeccion_prompt_id"), whatsapp_prospeccion_prompt_version: text(whatsapp, "prospeccion_prompt_version"), whatsapp_prospeccion_send_seller_data_to_customer: bool(whatsapp, "prospeccion_send_seller_data_to_customer") ?? false, whatsapp_prospeccion_followup_enabled: bool(whatsapp, "prospeccion_followup_enabled") ?? false, whatsapp_prospeccion_inactivity_minutes: number(whatsapp, "prospeccion_inactivity_minutes"), whatsapp_prospeccion_reengage_minutes: number(whatsapp, "prospeccion_reengage_minutes"), whatsapp_prospeccion_reengage_max_attempts: number(whatsapp, "prospeccion_reengage_max_attempts"), whatsapp_prospeccion_escalate_minutes: number(whatsapp, "prospeccion_escalate_minutes"), whatsapp_template_prospeccion_sids: text(whatsapp, "template_prospeccion_sids") }} /><TenantWhatsAppValidation tenantId={tenantId} /><WhatsAppAssistantSchedulePanel initialValues={scheduleResp.ok ? scheduleResp.data : null} /></>
      default:
