...
export async function GET() {
  ...
  const response = await fetch(`${baseUrl}/crm/catalog/items/export`, {
    headers: {
      Accept: "text/csv",
  ...
  headers.set("Content-Disposition", response.headers.get("Content-Disposition") ?? 'attachment; filename="productos.csv"')
