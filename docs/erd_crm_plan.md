...
4. **Seed de etapas con códigos canónicos:** _cubierto con `supabase/migrations/20260201_123000_seed_pipeline_stages.sql`; falta añadir un cron de verificación semanal que ejecute `SELECT id FROM organizaciones EXCEPT SELECT DISTINCT organizacion_id FROM etapas_pipeline WHERE lower(codigo) = 'captado'` y alerte si aparece algún tenant nuevo sin seeds._
