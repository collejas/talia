...
## 11. Panel y reportes
- [x] Mantener el endpoint único `/crm/asignaciones_vendedores`, que consume la vista y respeta `X-Organizacion-Id`.
- [x] Mostrar en Auditoría reasignaciones el estado del acuse (`Pendiente`/`Aceptada`), fecha y medio de aceptación.
- [x] Mostrar el historial de asignaciones en el panel (`/crm/asignaciones-vendedores`) con tabla y manejo de errores.
