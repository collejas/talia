-- Atribucion de plantillas de prospeccion por envio/conversacion.
-- No depende de metadata para la identidad estructural de la plantilla.
...
envios_base as (
    ...
    where e.organizacion_id = co.organizacion_id
      ...
      and (p_date_from is null or b.creado_en >= p_date_from)
      and (p_date_to is null or b.creado_en <= p_date_to)
),
template_catalog as (
    select
        eb.*,
...
conversation_rollup as (
    select
        ...
        coalesce(mt.template_id, null) as template_id,
        count(distinct coalesce(cc.conversacion_id, mt.conversacion_id))::bigint as conversaciones_total,
        count(distinct coalesce(cc.conversacion_id, mt.conversacion_id)) filter (where cc.respondio_en is not null)::bigint as conversaciones_respondidas,
    ...
    where (cc.campana_id is null or cc.campana_id = p_campana_id or p_campana_id is null)
...
select
    ...
    tc.version_id,
    sr.envios_totales,
    sr.mensajes_salientes,
    coalesce(cr.conversaciones_total, 0),
    coalesce(cr.conversaciones_respondidas, 0),
    coalesce(cr.oportunidades_total, 0),
    coalesce(cr.oportunidades_ganadas, 0)
from send_rollup sr
join public.campanas c on c.id = sr.campana_id
left join lateral (
    select tc2.template_nombre, tc2.template_slug, tc2.version_id
    from template_catalog tc2
    where tc2.campana_id = sr.campana_id and tc2.template_id is not distinct from sr.template_id
    limit 1
) tc on true
left join conversation_rollup cr
  on cr.campana_id = sr.campana_id
 and cr.template_id is not distinct from sr.template_id
order by sr.envios_totales desc, c.nombre, tc.template_nombre
...
comment on function public.prospeccion_campana_whatsapp_template_metricas_rango(uuid, timestamptz, timestamptz, integer, integer)
    is 'Metricas WhatsApp por campaña y plantilla usando columnas explicitas del envio y conversiones de la conversacion.';
