...
2. **Alertas de promoción fallida:** loggear métricas (Counter/OTel) cada vez que `promote_opportunity_stage` arroje `opportunity_stage_invalid_*` para detectar stages mal configurados por tenant; aprovechar `audit_logs` para dejar rastro cuando el sistema mueva una etapa automáticamente.
