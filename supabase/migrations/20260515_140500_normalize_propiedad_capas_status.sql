...
ALTER TABLE public.propiedad_capas
    ADD COLUMN IF NOT EXISTS status public.propiedad_status NOT NULL DEFAULT 'disponible';

COMMENT ON COLUMN public.propiedad_capas.status IS 'Estado operativo de la capa normalizado fuera de metadata.';

WITH extracted AS (
    SELECT
        c.id,
        CASE
            WHEN jsonb_typeof(c.metadata) = 'object' THEN
                CASE lower(NULLIF(btrim(c.metadata->>'status'), ''))
        ...
        CASE
            WHEN jsonb_typeof(c.metadata) = 'object' THEN
                COALESCE(c.metadata, '{}'::jsonb) - 'status' - 'estado'
            ELSE COALESCE(c.metadata, '{}'::jsonb)
        END AS cleaned_metadata
    FROM public.propiedad_capas c
)
UPDATE public.propiedad_capas c
SET
    status = COALESCE(c.status, extracted.status_from_metadata, 'disponible'::public.propiedad_status),
