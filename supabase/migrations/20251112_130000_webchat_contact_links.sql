...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'webchat_visitantes_contacto_fk'
          AND conrelid = 'public.webchat_visitantes'::regclass
    ) THEN
        ALTER TABLE public.webchat_visitantes
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'webchat_session_closures_contacto_fk'
          AND conrelid = 'public.webchat_session_closures'::regclass
    ) THEN
        ALTER TABLE public.webchat_session_closures
