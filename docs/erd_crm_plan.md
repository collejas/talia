...
- `backend/app/services/storage.py` todavía depende del service role para los RPC de mensajería (registrar/adjuntar mensajes de webchat y WhatsApp); la asociación con oportunidades ya pasa por `CRMRepository`, pero falta exponer esos RPC como endpoints `/crm` para completar el aislamiento multi-tenant. 【backend/app/services/storage.py:50-220】
...
1. **Cliente Supabase de servicio:** encapsular en `backend/app/repositories/crm_repository.py` todas las llamadas a Supabase usando el service role y el nuevo `organizacion_id`. _Listo; sólo restan los RPC específicos de mensajería (registrar mensajes, adjuntos) que siguen colgándose de `storage.py`._
...
4. **Sincronización de conversaciones:** migrar los RPC de mensajería y bitácoras (`registrar_mensaje_webchat`, adjuntos, etc.) para que `storage.py` invoque helpers equivalentes en `/crm`, completando el corte con las tablas legacy.
