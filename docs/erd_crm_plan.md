...
- Las automatizaciones de conversación siguen usando `storage.ensure_lead_tarjeta`, pero la función ahora crea/actualiza oportunidades del CRM en lugar de `lead_tarjetas`, registrando el `conversation_id` y el canal dentro del metadata de la oportunidad. Falta exponer esta operación como helper en `/crm`, pero la información ya fluye al pipeline nuevo. 【backend/app/services/storage.py:1792-2052】
...
- `backend/app/services/storage.py` todavía depende del service role para crear/actualizar oportunidades cuando llega una conversación nueva (`ensure_lead_tarjeta`) y para registrar mensajes; aunque ahora escribe en `oportunidades`, falta encapsularlo en `CRMRepository` o exponer helpers REST para que todos los flujos pasen por `/crm`. 【backend/app/services/storage.py:1792-2052】
- El router `/crm` y `CRMRepository` ya abastecen agenda, visitas, prospección, settings, contactos, inbox y cotizaciones, pero los procesos automáticos (webchat/WhatsApp) siguen hablando directo con Supabase vía `storage.py`. Migrar esos trayectos permitiría aplicar RLS multi-tenant y observabilidad homogénea. 【backend/app/services/storage.py:150-2052】
