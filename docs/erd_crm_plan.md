...
3. **Migración definitiva de `lead_tarjetas_auto_precalificado`:** reemplazar el trigger Postgres por una tarea que revise oportunidades con metadata `legacy_lead_id` y elimine dependencias de `lead_etapas`. Una vez que todos los canales usen `_maybe_capture_lead`, podemos desactivar `_lead_tarjeta_auto_precalificar`.
