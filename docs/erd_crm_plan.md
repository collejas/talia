...
- **Pendientes inmediatos:** (1) propagar el nuevo flujo a los módulos que aún dependen de `panel.py` (prospección) para cerrar completamente el modelo legacy; (2) agregar manejo de conflictos de etapa (`409 opportunity_stage_conflict`) en la UI para soportar edición concurrente; (3) enlazar los contadores de visitantes (`visitantes_sin_chat`) con los dashboards ahora que `/crm/visitas/*` expone los mismos datos.
