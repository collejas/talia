...
- `cuentas`: empresas/organizaciones con `tipo`, industria, tamaño, sitio web, dirección o `direccion JSONB`, `propietario_usuario_id` y opcional `ubicacion_geom` si se usa PostGIS para persistir geometrías.
...
5. [x] Implementar `tickets` y `ticket_comentarios` si aplica al soporte actual. _`/crm/tickets` y `/crm/tickets/{id}/comentarios` operativos con RLS y restringidos al owner/admin del tenant maestro; falta migrar tickets existentes y adoptar la nueva API en el panel._
...
- Prospección (Google Places y DENUE) expone `/crm/prospeccion/*` y los clientes (`frontend/panel/src/lib/prospeccion/*`) consumen esos endpoints mediante los proxies `/api/prospeccion/*`, por lo que ya no se llama a Supabase directo para crear/listar/borrar búsquedas y resultados; además, el router legacy fue eliminado junto con `/panel/env.js`, así que la única superficie expuesta es `/crm`. 【backend/app/api/routes/crm.py:2139-2491】【backend/app/repositories/crm.py:1572-1664】【backend/app/main.py:1-120】
...
- **Backend:** FastAPI expone `/crm` con un cliente Supabase de servicio y ya cubre pipeline, archivos, notas, clientes/portal, cotizaciones, inbox, agenda, visitas y prospección. El router legacy del panel se retiró por completo (incluyendo `/panel/env.js`) y sólo permanece el montaje de assets compartidos; las únicas llamadas directas pendientes viven en `storage.py`, donde sobreviven reportes masivos/descargas específicas que aún no tienen su contraparte en `/crm`. 【backend/app/services/storage.py:900+】【backend/app/main.py:1-120】
...
- **Analytics de catálogo:** las tarjetas del dashboard consultan `/crm/analytics/catalog/ventas|embudo` mediante `callCrmApi`, y los endpoints legacy `/analytics/catalog/*` se eliminaron junto con el router del panel; incluso el CSV se genera en el server action usando los datos del CRM para mantener la separación multi-tenant. 【backend/app/api/routes/crm.py:280-600】【frontend/panel/src/app/api/catalog/items/route.ts:1-48】
...
- **Limpieza del panel:** el router legacy y los bundles estáticos (`backend/app/public/panel*`) fueron eliminados, por lo que cualquier ajuste vive exclusivamente en `/crm` y el backend ya no expone `/panel`. 【backend/app/public†】【backend/app/main.py:1-120】
...
3. **Frontend panel + portal público**
   - [x] Actualizar `frontend/panel/src/lib/clientes/data.ts` y las vistas de clientes para consumir `cuentas` y `clientes` CRM sin referencias a `lead_tarjeta_id`. Revisar formularios (actualización fiscal, documentos, responsables) para que envíen los payloads nuevos.
   - [x] Modificar los SDK/API del portal (`frontend/panel/src/lib/portal/data.ts` y `/api/portal/*`) para alinearlos con los cambios en el backend y validar expiración de tokens usando los campos actualizados.

    Se expuso `GET /crm/clientes` sobre el repositorio central para devolver el `_cliente_select_clause` con documentos y responsables filtrados por `organizacion_id`, lo que permite que el panel liste clientes reales sin depender de metadata en cuentas. 【backend/app/api/routes/crm.py:2791-2864】【backend/app/repositories/crm.py:2635-2695】 El server action `loadClientesData` ahora consume ese endpoint y normaliza los registros en vez de reconstruirlos desde `metadata.legacy_lead_id`, por lo que las tarjetas del módulo de clientes muestran los campos CRM reales. 【frontend/panel/src/lib/clientes/data.ts:4-214】 En paralelo, el SDK del portal valida localmente enlaces vencidos o revocados y los proxies `/api/portal/*` devuelven mensajes claros cuando Supabase marca un token como expirado, alineando la UX con los nuevos campos de `cliente_portal_tokens`. 【frontend/panel/src/lib/portal/data.ts:1-40】【frontend/panel/src/app/api/portal/[token]/route.ts:1-74】
4. **Limpieza final**
   - [x] Retirar triggers/vistas/policies relacionadas a `lead_tarjetas` en Supabase una vez que el pipeline CRM tenga doble escritura y auditoría completada. _`supabase/migrations/20260730_120000_drop_legacy_leads.sql` elimina las tablas `lead_*` junto con las vistas/funciones heredadas y deja `clientes.legacy_lead_id` sólo como referencia interna._
   - [x] Desmontar los bundles estáticos legacy (`backend/app/public/panel*`) y cerrar `/panel/env.js` para que sólo quede operando el frontend moderno vía `/crm`. 【backend/app/main.py:1-120】
