...
\restrict 7vcB5HaEaHbtdCZH9sMV0im4uDu7G56Eck3Eung5H7RAhLCMkaW75ajN4eRnBds
...
CREATE TABLE public.citas (
    ...
    CONSTRAINT citas_provider_check CHECK ((provider = ANY (ARRAY['hosting'::text, 'google'::text]))),
...
--
-- Name: FUNCTION fn_cita_upsert(p_id uuid, p_tarjeta_id uuid, p_contacto_id uuid, p_conversacion_id uuid, p_start_at timestamp with time zone, p_end_at timestamp with time zone, p_timezone text, p_estado public.cita_estado, p_provider text, p_provider_calendar_id text, p_provider_event_id text, p_meeting_url text, p_location text, p_notes text, p_metadata jsonb, p_cancel_reason text, p_created_by uuid, p_updated_by uuid, p_merge_metadata boolean, p_expected_updated_at timestamp with time zone, p_remove_provider_event boolean, p_reminder_sent_at timestamp with time zone, p_reminder_status text, p_external_join_url text, p_scheduled_via text); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.fn_cita_upsert(p_id uuid, p_tarjeta_id uuid, p_contacto_id uuid, p_conversacion_id uuid, p_start_at timestamp with time zone, p_end_at timestamp with time zone, p_timezone text, p_estado public.cita_estado, p_provider text, p_provider_calendar_id text, p_provider_event_id text, p_meeting_url text, p_location text, p_notes text, p_metadata jsonb, p_cancel_reason text, p_created_by uuid, p_updated_by uuid, p_merge_metadata boolean, p_expected_updated_at timestamp with time zone, p_remove_provider_event boolean, p_reminder_sent_at timestamp with time zone, p_reminder_status text, p_external_join_url text, p_scheduled_via text) IS 'Inserta o actualiza citas aplicando permisos, duración predeterminada y merge opcional de metadatos.';
...
\unrestrict 7vcB5HaEaHbtdCZH9sMV0im4uDu7G56Eck3Eung5H7RAhLCMkaW75ajN4eRnBds
