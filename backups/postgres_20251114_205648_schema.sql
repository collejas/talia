...
\restrict HwFVeaFSQjvYdtYPCj32XFuGZjmSehazEXFfjKtj7h4j02O7V9GFtYgGxWSnMJe
...
--
-- Name: fn_calendar_hold_slot(uuid, timestamp with time zone, uuid, uuid, integer, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_calendar_hold_slot(p_resource_id uuid, p_slot_start timestamp with time zone, p_conversacion_id uuid, p_contact_id uuid DEFAULT NULL::uuid, p_hold_minutes integer DEFAULT 5, p_metadata jsonb DEFAULT '{}'::jsonb) RETURNS TABLE(hold_id uuid, resource_id uuid, slot_start timestamp with time zone, slot_end timestamp with time zone, expires_at timestamp with time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
--
-- Name: FUNCTION fn_calendar_hold_slot(p_resource_id uuid, p_slot_start timestamp with time zone, p_conversacion_id uuid, p_contact_id uuid, p_hold_minutes integer, p_metadata jsonb); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.fn_calendar_hold_slot(p_resource_id uuid, p_slot_start timestamp with time zone, p_conversacion_id uuid, p_contact_id uuid, p_hold_minutes integer, p_metadata jsonb) IS 'Bloquea temporalmente un slot disponible mientras el visitante confirma la cita.';
...
\unrestrict HwFVeaFSQjvYdtYPCj32XFuGZjmSehazEXFfjKtj7h4j02O7V9GFtYgGxWSnMJe
