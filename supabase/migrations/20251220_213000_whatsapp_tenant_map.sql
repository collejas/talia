-- Permite especificar la organización al registrar mensajes de WhatsApp.
...
BEGIN
    ...
    IF p_contact_id IS NOT NULL THEN
        SELECT id, organizacion_id INTO v_contact_id, v_org FROM public.contactos WHERE id = p_contact_id;
        IF NOT FOUND THEN
    ...
    IF p_conversation_id IS NOT NULL THEN
        ...
        IF v_org IS NULL THEN
            SELECT organizacion_id INTO v_org FROM public.contactos WHERE id = v_contact_id;
        END IF;
    END IF;

    IF v_contact_id IS NULL AND COALESCE(p_whatsapp_id, '') <> '' THEN
        SELECT ic.contacto_id, ct.organizacion_id
          INTO v_contact_id, v_org
...
$function$;
    ...
    ) IS 'Registra mensajes del canal WhatsApp permitiendo especificar el tenant destino.';
END;
$$;
