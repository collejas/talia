...
-- Cambia la firma/retorno de la función: es necesario dropear primero
-- Elimina versiones previas para permitir cambiar firma y retorno
DROP FUNCTION IF EXISTS public.registrar_mensaje_webchat(text, text, text, text, jsonb);
DROP FUNCTION IF EXISTS public.registrar_mensaje_webchat(text, text, jsonb, text, text);
DROP FUNCTION IF EXISTS public.registrar_mensaje_webchat(text, text, text, text, jsonb, integer);
DROP FUNCTION IF EXISTS public.registrar_mensaje_webchat(text, text, jsonb, text, text, integer);

-- Regla de "nueva conversación" si la última actividad supera 24 horas.
-- También propagamos el conversation_id de OpenAI (conv_...) a la tabla conversaciones.
