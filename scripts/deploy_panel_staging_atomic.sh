...
ensure_free_space() {
  ...
  if [[ -z "${avail_gb}" ]]; then
    echo "[deploy-staging] WARN no fue posible medir espacio libre."
    return 0
  fi
  if (( avail_gb < MIN_FREE_GB )); then
    echo "[deploy-staging] ERROR espacio insuficiente: ${avail_gb}G libres, minimo requerido=${MIN_FREE_GB}G."
    echo "[deploy-staging] Ejecuta scripts/cleanup_disk.sh o reduce KEEP_RELEASES/KEEP_BACKUPS antes de continuar."
    return 1
  fi
  echo "[deploy-staging] Espacio libre OK: ${avail_gb}G"
}
...
wait_for_api_health() {
  ...
  echo "[deploy-staging] Esperando salud de API en ${API_HEALTH_URL} (timeout=${API_HEALTH_TIMEOUT_SECONDS}s)"
  while (( SECONDS < deadline )); do
    if curl -fsS --max-time 2 "${API_HEALTH_URL}" >/dev/null 2>&1; then
      echo "[deploy-staging] API saludable"
      return 0
    fi
    sleep 1
  done
  echo "[deploy-staging] ERROR API no reporto salud a tiempo: ${API_HEALTH_URL}"
  return 1
}
