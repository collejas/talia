...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const response = await callCrmApi<UnknownRecord[]>(
    `/crm/cuentas/${encodeURIComponent(cuentaId)}/direcciones${qs}`,
    {
      method: "GET",
      withUserToken: true,
    },
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_direcciones_list_failed" },
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const response = await callCrmApi<UnknownRecord>(
    `/crm/cuentas/${encodeURIComponent(cuentaId)}/direcciones`,
    {
      method: "POST",
      body: payload,
      withUserToken: true,
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_direccion_create_failed" },
      { status: response.status ?? 502 },
    );
  }
