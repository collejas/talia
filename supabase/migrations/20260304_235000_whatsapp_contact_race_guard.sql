...
BEGIN
    ...
    v_contact_lock_key := CONCAT_WS(
        '|',
        'whatsapp_contact',
        COALESCE(v_org::text, 'orgless'),
        COALESCE(NULLIF(p_whatsapp_id, ''), NULLIF(p_phone_e164, ''), NULLIF(v_metadata->>'wa_id', ''))
    );
    PERFORM pg_advisory_xact_lock(hashtext('whatsapp_contact_lock'), hashtext(v_contact_lock_key));
    ...
    IF v_contact_id IS NULL AND COALESCE(p_whatsapp_id, '') <> '' THEN
        SELECT ic.contacto_id, ct.organizacion_id
         ...
         WHERE ic.canal = 'whatsapp'
           ...
           AND (v_org IS NULL OR ct.organizacion_id = v_org)
