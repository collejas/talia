...
- **Captura automática al primer dato:** los tool calls `set_email` y `set_phone_number` del asistente de webchat invocan `storage.capture_lead_if_ready`, que crea/recupera la oportunidad vía `storage.ensure_lead_tarjeta` y promueve la etapa cuando ya hay al menos un medio de contacto. Esto garantiza que las tarjetas de webchat se vean en el embudo sin depender de `lead_tarjetas`. 【backend/app/assistants/tools/lead.py:44-125】【backend/app/services/storage.py:921-991】
...
- **Integración con canales existentes:** la lógica reutiliza `ensure_conversation_opportunity`, así que WhatsApp/voz sólo necesitan invocar `capture_lead_if_ready` después de actualizar el contacto para alinearse con el nuevo pipeline; no hace falta tocar Postgres ni los RPC legacy.
...
- **Monitor de seeds:** `supabase/migrations/20260201_124500_view_missing_pipeline_stages.sql` crea la vista `organizaciones_missing_etapas_pipeline`, que devuelve un registro por organización y código faltante. Esto se puede enganchar a un cron (por ejemplo `select * from organizaciones_missing_etapas_pipeline`) para alertar cuando un tenant nuevo no tenga `captado`.
- **Observabilidad de auto-promoción:** `capture_lead_if_ready` y `promote_opportunity_stage` emiten eventos estructurados (`capture_lead.*`, `promote_stage.*`) cada vez que se crea o ignora una oportunidad automática. Estos logs sirven para armar métricas/alertas sobre leads sin datos de contacto, stages faltantes o promociones fallidas.
...
1. **Propagar `capture_lead_if_ready` a WhatsApp y al formulario de landing:** llamar al helper después de `set_email`/`set_phone_number` en `backend/app/channels/whatsapp/tools.py` y en cualquier server action que complete datos de contacto para que todos los canales compartan la misma lógica de promoción.
2. **Alertas de promoción fallida:** aprovechar los eventos `capture_lead.*` / `promote_stage.*` para generar métricas o alertas (Counter/OTel) cada vez que falle la promoción automática y reflejarlo en `audit_logs`, garantizando trazabilidad por tenant.
3. **Migración definitiva de `lead_tarjetas_auto_precalificado`:** _Listo en `supabase/migrations/20260201_120000_disable_legacy_lead_triggers.sql`, que deshabilita los triggers `lead_tarjetas_*` y `contactos_auto_precalificado` tras mover la lógica al backend. Mantén la tabla en modo solo lectura por si se requieren consultas históricas._
4. **Seed de etapas con códigos canónicos:** _cubierto con `supabase/migrations/20260201_123000_seed_pipeline_stages.sql` y la vista `organizaciones_missing_etapas_pipeline`; programa un cron (cada hora o día) que ejecute `select * from organizaciones_missing_etapas_pipeline` y alerte si devuelve filas para intervenir el tenant._
5. **UI de auditoría en el embudo:** mostrar un badge “Movido automáticamente” cuando el backend cambie la etapa con `source=assistant`, usando la metadata que escribimos en `oportunidad_etapas_historial`. Esto facilita explicar a los vendedores por qué apareció una tarjeta en “Captado” sin intervención manual.
