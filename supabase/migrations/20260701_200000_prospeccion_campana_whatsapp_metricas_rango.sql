...
mensajes_campana as (
    select
        mb.*,
        coalesce(pb.campana_id, mb.campana_id) as campana_resuelta_id
    from mensajes_base mb
    left join public.prospeccion_contacto_batch pb
      on pb.id = mb.batch_id
     and pb.organizacion_id = (select organizacion_id from contexto_org)
    left join public.campanas c
      on c.id = coalesce(pb.campana_id, mb.campana_id)
...
conversation_rollup as (
    select
        cm.campana_id,
        count(*)::bigint as conversaciones_total,
        count(*) filter (
            where exists (
                select 1
                from mensajes_campana mc2
                where mc2.conversacion_id = cm.conversacion_id
                  and mc2.direccion = 'entrante'
            )
...
message_rollup as (
    select
        campana_resuelta_id as campana_id,
...
comment on function public.prospeccion_campana_whatsapp_metricas_rango(uuid, timestamptz, timestamptz, integer, integer)
    is 'Resume metricas de campañas WhatsApp por organización, con mensajes, conversaciones, oportunidades y lotes.';
