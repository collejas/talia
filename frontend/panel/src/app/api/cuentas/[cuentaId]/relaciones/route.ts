...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const response = await callCrmApi<UnknownRecord[]>(
    `/crm/cuentas/${encodeURIComponent(cuentaId)}/relaciones${qs}`,
    {
      method: "GET",
      withUserToken: true,
    },
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_relaciones_list_failed" },
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const response = await callCrmApi<UnknownRecord>(
    `/crm/cuentas/${encodeURIComponent(cuentaId)}/relaciones`,
    {
      method: "POST",
      body: payload,
      withUserToken: true,
  ...
  if (!response.ok) {
    return NextResponse.json(
      { error: response.error || "cuenta_relacion_create_failed" },
      { status: response.status ?? 502 },
    );
  }
