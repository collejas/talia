-- Ajustes adicionales para el flujo de WhatsApp multitenant:
-- 1) Propaga organizacion_id a webhooks_entrantes, conversaciones y mensajes.
-- 2) Agrega políticas RLS para permitir al rol postgres operar usando el tenant del contexto.
...
-- Garantiza unicidad de (canal,id_externo) para soportar ON CONFLICT.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'identidades_canal_canal_id_externo_key'
    ) THEN
...
-- Políticas adicionales para permitir el acceso del rol postgres usando el tenant del contexto.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'conversaciones' AND policyname = 'conversaciones_rpc_access'
    ) THEN
        CREATE POLICY conversaciones_rpc_access ON public.conversaciones
            FOR ALL
            TO postgres
            USING (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid)
            WITH CHECK (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid);
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'mensajes' AND policyname = 'mensajes_rpc_access'
    ) THEN
        CREATE POLICY mensajes_rpc_access ON public.mensajes
            FOR ALL
            TO postgres
            USING (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid)
            WITH CHECK (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid);
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'webhooks_entrantes' AND policyname = 'webhooks_rpc_access'
    ) THEN
        CREATE POLICY webhooks_rpc_access ON public.webhooks_entrantes
            FOR ALL
            TO postgres
            USING (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid)
            WITH CHECK (organizacion_id = NULLIF(current_setting('app.current_organizacion_id', true), '')::uuid);
