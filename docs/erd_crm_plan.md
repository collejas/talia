...
4. **Seed de etapas con códigos canónicos:** asegurar que cada tenant tenga los códigos `captado`, `precalificado`, `demo`, etc., en `etapas_pipeline`. Preparar un script de verificación (Supabase function o management command) que normalice `metadata->>legacy_codigo` para las organizaciones creadas antes de 2024 y así explotar `get_stage_by_code`.
