-- =========================================================
-- PUI SaaS Multitenant Base Schema
-- PostgreSQL 14+
-- =========================================================
...
create table if not exists tenants (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint chk_tenants_rfc_format
...
create table if not exists tenant_addresses (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (tenant_id, address_type, is_primary) deferrable initially immediate
);
...
create table if not exists biometric_access_events (
  ...
  access_type varchar(50) not null, -- view/download/re-encrypt/delete
