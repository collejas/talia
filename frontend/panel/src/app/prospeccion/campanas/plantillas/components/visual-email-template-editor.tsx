...
type Block = { id: string; kind: BlockKind; title: string; content: string; imageId?: string; href?: string }
...
function initialBlocks(html: string): Block[] {
...
function blockToHtml(block: Block, assets: LogoAsset[]): string {
  ...
  if (block.kind === "columns") return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:16px;border:1px solid #eceef3;border-radius:14px;"><strong>Encuentra clientes</strong><br /><span>Prospecta empresas y contactos.</span></td><td style="padding:16px;border:1px solid #eceef3;border-radius:14px;"><strong>Da seguimiento</strong><br /><span>Organiza todo en tu CRM.</span></td></tr></table>`
...
export function VisualEmailTemplateEditor({ value, assets, onChange }: Props) {
  const [blocks, setBlocks] = useState<Block[]>(() => initialBlocks(value))
    ...
    void fetch("/api/prospeccion/plantillas/ai?canal=correo", { cache: "no-store" })
      ...
      .then((payload) => {
        if (!cancelled && Array.isArray(payload?.items) && payload.items.length) {
          setVariables(payload.items.map((item: { clave?: string }) => ({
            clave: item.clave ?? "",
            etiqueta: LABELS[item.clave ?? ""] ?? item.clave ?? "",
  ...
  return (
    <div className="overflow-hidden rounded-2xl border bg-[#f6f7fb] shadow-sm">
      <div className="flex min-h-16 flex-wrap items-center gap-3 border-b bg-white px-4 py-3">
        ...
        <div className="ml-auto flex gap-2"><Button type="button" variant="outline" size="sm">Vista previa</Button><span className="rounded-full bg-emerald-50 px-2.5 py-1.5 text-xs text-emerald-700">✓ Guardado</span></div>
      </div>
      <div className="grid min-h-[680px] lg:grid-cols-[220px_minmax(0,1fr)_240px]">
        ...
        <section className="overflow-auto p-4 sm:p-6">
          ...
          <div className={`mx-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_14px_40px_rgba(25,30,55,.08)] transition-all sm:p-8 ${mobile ? "max-w-[390px]" : "max-w-[720px]"}`}>
            {blocks.map((block) => <button key={block.id} type="button" className={`group relative mb-2 block w-full rounded-xl border p-3 text-left ${selected?.id === block.id ? "border-violet-500 bg-violet-50/40 ring-4 ring-violet-500/10" : "border-transparent hover:border-violet-200"}`} onClick={() => setSelectedId(block.id)}>
              ...
              {block.kind === "image" ? (block.imageId && assets.find((asset) => asset.id === block.imageId) ? <Image src={assets.find((asset) => asset.id === block.imageId)?.file_url ?? ""} alt="" width={600} height={180} className="mx-auto max-h-40 w-full object-contain" unoptimized /> : <div className="grid h-28 place-items-center rounded-xl bg-slate-100 text-xs text-slate-400">Logo o fotografía</div>) : block.kind === "divider" ? <div className="h-px bg-slate-200" /> : block.kind === "space" ? <div className="h-10" /> : block.kind === "button" ? <div className="text-center"><span className="inline-flex rounded-xl bg-[#171923] px-5 py-3 text-sm font-bold text-white">{block.content}</span></div> : block.kind === "columns" ? <div className="grid grid-cols-2 gap-2"><div className="rounded-xl border bg-slate-50 p-3 text-xs font-semibold">Encuentra clientes</div><div className="rounded-xl border bg-slate-50 p-3 text-xs font-semibold">Da seguimiento</div></div> : <div><h3 className="text-xl font-bold">{block.title}</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{block.content}</p></div>}
            </button>)}
          </div>
        </section>
        <aside className="border-t bg-white p-4 lg:border-l lg:border-t-0">
          ...
          {selected?.kind === "button" ? <div className="space-y-3"><div className="space-y-2"><Label>Texto</Label><Input value={selected.content} onChange={(event) => updateSelected({ content: event.target.value })} /></div><div className="space-y-2"><Label>Enlace</Label><Input value={selected.href ?? ""} onChange={(event) => updateSelected({ href: event.target.value })} /></div></div> : null}
      ...
      <div className="flex justify-center gap-2 border-t bg-white p-3"><Button type="button" variant="outline" size="sm">✉ Enviar prueba</Button><Button type="button" variant="outline" size="sm" onClick={() => setMobile(true)}>▯ Ver en móvil</Button></div>
    </div>
  )
}
