...
1. **Alertas de promoción fallida:** aprovechar los eventos `capture_lead.*` / `promote_stage.*` para generar métricas o alertas (Counter/OTel) cada vez que falle la promoción automática y reflejarlo en `audit_logs`, garantizando trazabilidad por tenant.
