...
- [x] { } Documentar que la vector store se reindexa y que la auditoría respalda esa sincronización al asistente, sin mostrar una card específica.
- [x] { } Añadir enlaces directos desde el asistente a los registros correspondientes (por ID/slug).
- [x] { } Guiar la conversación siguiendo la jerarquía líneas → familias/modelos → productos y redactar las referencias en lenguaje natural (sin URLs crudas).
