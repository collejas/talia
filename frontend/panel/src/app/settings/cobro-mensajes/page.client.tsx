...
type BillingMessage = {
  ...
  categoria_interna_cobro?: string
  estado_proveedor: string
  cargo_app_importe: number | string
  costo_meta_importe: number | string
  costo_meta_estado: "pendiente" | "confirmado" | "no_aplica" | "no_entregado"
  meta_entregado_en?: string | null
  costo_total_mensaje: number | string
  creado_en: string
  mensaje_creado_en: string
  organizacion_nombre?: string | null
  periodo_label?: string | null
  contacto_nombre?: string | null
  contacto_telefono?: string | null
  contacto_correo?: string | null
  operativo_eliminado?: boolean
  operativo_eliminado_en?: string | null
}
...
export function MessageBillingPageClient({ isOwner }: { isOwner: boolean }) {
  ...
  const exportUrl = `/api/billing${scopePrefix}/messages/export${summaryQuery ? `?${summaryQuery}` : ""}`
  ...
  return (
    <div className="flex flex-col gap-6 px-4 py-2 lg:px-6">
      ...
      {loading ? <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-xl" />)}</div> : s ? <section aria-labelledby="billing-kpis-title" className="space-y-3"><h2 id="billing-kpis-title" className="text-xl font-semibold">KPI&apos;s</h2><div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
        ...
        <KpiCard label="Costo Meta confirmado" value={money(s.costo_meta_total)} detail="Solo entregados y billable; lo paga el tenant a Meta" icon={IconSend} />
      ...
      <Card><CardHeader><CardTitle>Detalle de tarifas</CardTitle><CardDescription>El cargo GEOACTIV depende de la aceptación; el costo Meta solo aparece cuando la entrega es confirmada.</CardDescription></CardHeader><CardContent>
        {messageLoading ? <Skeleton className="h-48 w-full" /> : messages?.items.length ? <><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Fecha</TableHead>{isOwner ? <TableHead>Tenant</TableHead> : null}<TableHead>Periodo</TableHead><TableHead>Contacto</TableHead><TableHead>Dirección</TableHead><TableHead>Meta</TableHead><TableHead>Estado</TableHead><TableHead className="text-right">GEOACTIV</TableHead><TableHead className="text-right">Meta</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{messages.items.map((item) => <TableRow key={item.id}><TableCell className="whitespace-nowrap">{new Date(item.mensaje_creado_en || item.creado_en).toLocaleString("es-MX")}</TableCell>{isOwner ? <TableCell className="max-w-40"><span className="block truncate" title={item.organizacion_nombre ?? undefined}>{item.organizacion_nombre || "Tenant no identificado"}</span></TableCell> : null}<TableCell className="whitespace-nowrap text-xs">{item.periodo_label || "Periodo no disponible"}</TableCell><TableCell className="max-w-48"><div className="flex items-center gap-2"><span className="block truncate" title={item.contacto_nombre ?? undefined}>{item.contacto_nombre || (item.operativo_eliminado ? "Contacto eliminado" : "Contacto no identificado")}</span>{item.operativo_eliminado ? <Badge variant="outline">Eliminado</Badge> : null}</div>{item.contacto_telefono || item.contacto_correo ? <span className="block truncate text-xs text-muted-foreground">{item.contacto_telefono || item.contacto_correo}</span> : null}</TableCell><TableCell><Badge variant={item.direccion === "saliente" ? "default" : "secondary"}>{item.direccion}</Badge></TableCell><TableCell><div>{metaCategoryLabel(item.categoria_meta)}</div><span className="text-xs text-muted-foreground">{item.proveedor} · {item.canal}</span></TableCell><TableCell>{item.estado_proveedor}</TableCell><TableCell className="text-right">{money(item.cargo_app_importe)}</TableCell><TableCell className="text-right"><div>{money(item.costo_meta_importe)}</div><span className="text-xs text-muted-foreground">{item.costo_meta_estado === "confirmado" ? "Confirmado" : item.costo_meta_estado === "no_entregado" ? "No entregado" : item.costo_meta_estado === "no_aplica" ? "No aplica" : "Pendiente"}</span></TableCell><TableCell className="text-right font-medium">{money(item.costo_total_mensaje)}</TableCell></TableRow>)}</TableBody></Table></div><div className="mt-4 flex items-center justify-between text-sm text-muted-foreground"><span>{integer(messages.total)} mensajes</span><div className="flex items-center gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Anterior</Button><span>Página {page} de {totalPages}</span><Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>Siguiente</Button></div></div></> : <div className="py-12 text-center text-sm text-muted-foreground">Aún no hay mensajes contabilizados con estos filtros.</div>}
      </CardContent></Card>
    </div>
  )
}
