...
-- Reconcilia únicamente cargos Meta que siguen en unknown y cuyo WAMID ya
-- tiene un callback vinculado con pricing.category válido. No crea cargos y
-- no utiliza dirección/origen como sustituto de la información de Meta.
...
BEGIN
    ...
    LOOP
        ...
        IF COALESCE(v_result.actualizado, false) THEN
            ...
            v_delta := v_delta + COALESCE(v_result.costo_meta_importe, 0);
        END IF;
    END LOOP;
...
REVOKE ALL ON FUNCTION public.reconcile_meta_unknown_billing(integer)
    FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reconcile_meta_unknown_billing(integer)
    TO service_role;

COMMENT ON FUNCTION public.reconcile_meta_unknown_billing(integer) IS
    'Reconciliación idempotente de categorías Meta desde callbacks vinculados; no infiere pricing ni crea cargos.';
