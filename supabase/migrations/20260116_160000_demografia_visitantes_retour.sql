...
conversation_geo AS (
    SELECT
        ...
        COALESCE(
            ...
            CASE
                WHEN cb.telefono_e164 ~ '^\\+?52' THEN
                    CASE
                        WHEN length(regexp_replace(cb.telefono_e164, '\\D', '', 'g')) >= 13
                             AND substring(regexp_replace(cb.telefono_e164, '\\D', '', 'g') FROM 3 FOR 1) = '1'
                            THEN substring(regexp_replace(cb.telefono_e164, '\\D', '', 'g') FROM 4 FOR 3)
                        ELSE substring(regexp_replace(cb.telefono_e164, '\\D', '', 'g') FROM 3 FOR 3)
                    END
                ELSE NULL
            END
        ) AS raw_cve_ent,
        COALESCE(
            ...
            CASE WHEN cb.telefono_e164 ~ '^\\+?52' THEN 'México' END
        ) AS raw_nom_ent,
        COALESCE(
...
SELECT
    ...
    mu.location_key,
    mu.location_name,
    (SUM(mu.total_visitas) + SUM(mu.whatsapp_total) + SUM(mu.voz_total))::bigint AS total_visitas,
